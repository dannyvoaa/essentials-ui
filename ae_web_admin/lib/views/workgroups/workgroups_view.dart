...
import 'package:american_essentials_web_admin/api/api_workgroups_delete.dart';
import 'package:american_essentials_web_admin/api/api_workgroups_fetch.dart';
...
class WorkgroupsView extends StatefulWidget {
  static String routeId = 'WorkgroupsView';

  // Setup any required variables
  final Map<String, Object> payload;

  // Initialize the view
  WorkgroupsView({this.payload});

  @override
  _WorkgroupsViewState createState() => _WorkgroupsViewState();
}

class _WorkgroupsViewState extends State<WorkgroupsView> {
  // Setup any required variables
  List<WorkgroupsModel> _listWorkgroups = List<WorkgroupsModel>();
  ...
  Future<void> _reloadData() async {
    ...
    Future.microtask(() async {
      ...
      Future.delayed(Duration(milliseconds: 250), () async {
        // Reset the list of workgroups
        _listWorkgroups.clear();

        // Request data from the API
        ApiWorkgroupsFetchResult apiWorkgroupsFetchResult =
            await ApiWorkgroupsFetch.sendRequest();

        // Execute the API Helper
        ApiHelper.execute(
          buildContext: context,
          boolDismissProcessingOnError: true,
          httpStatusCode: apiWorkgroupsFetchResult.statusCode,
          onSuccess: () {
            // Populate the list with event data from the API
            _listWorkgroups = apiWorkgroupsFetchResult.data;
  ...
  List<Widget> _reloadListView() {
    ...
    // Check to see if _listWorkgroups has data
    if (_listWorkgroups.length > 0) {
      // Iterate through each WorkgroupsModel in the list
      for (WorkgroupsModel workgroupsModel in _listWorkgroups) {
        // Add an item to the list of widgets
        listWidgets.add(
          // Add a new basic cell
          BasicCell(
            stringId: workgroupsModel.id,
            stringLabel: workgroupsModel.label,
            onDeleteAction: () {
              // Confirm that the user would like to delete this event
              this._deleteAction(
                buildContext: context,
                workgroupsModel: workgroupsModel,
              );
            },
            onEditAction: () {
      ...
      listWidgets.add(
        ErrorCell(
          stringMessage:
              'No workgroups were found or the network connection is unavailable.',
        ),
      );
    }
  ...
  void _deleteAction({
    ...
    if (dialogResult.toString().toLowerCase() == 'delete') {
      ...
      ApiWorkgroupsDeleteResult apiWorkgroupsDeleteResult =
          await ApiWorkgroupsDelete.sendRequest(
        workgroupsModel: workgroupsModel,
      );
      ...
      ApiHelper.execute(
        buildContext: context,
        boolDismissProcessingOnError: true,
        httpStatusCode: apiWorkgroupsDeleteResult.statusCode,
        onSuccess: () {
          // Hide the processing view
          Processing.dismiss(buildContext: context);
  ...
  void _editAction({
    ...
    Transitions.pushReplacementWithFade(
      buildContext: context,
      stringRouteId: WorkgroupsAddView.routeId,
      payload: {'workgroupsModel': workgroupsModel},
    );
  }
}
