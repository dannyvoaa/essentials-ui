...
class Machine<S, E> {
  ...
  factory Machine({String name, S rootId, Logger log}) {
  ...
  /// If any child handles an event, the message processing for the event done.
  /// Any guard or action functions trigger further events, they will be queued
  /// till the currently processing event and any transition completed.
  ///
...
class State<S, E> {
  ...
  bool get isActive =>
      parent?.active == this ||
      (parent is ParallelState && parent.isActive) ||
      (isRoot && _hsm.isRunning);
  ...
  State(this.id, {this.parent, Logger log})
    ...
    if (_hsm != null) {
      if (_hsm[id] != null) {
        throw ArgumentError('$_hsm: already contains id:$id = ${_hsm[id]} '
            'path:${_hsm[id].path}');
      }
      _hsm._states[id] = this;
    }
    if (parent != null) {
  ...
  List<EventHandler<S, E>> addHandler(E event,
          {State<S, E> target,
          GuardFunction<E> guard,
          ActionFunction<E> action,
          bool local = true}) =>
  ...
  List<EventHandler<S, E>> addHandlers(
          E event, List<EventHandler<S, E>> handlers) =>
  ...
  bool _handle(E event, dynamic data) {
    ...
    for (var handler in handlers[event]) {
      ...
      if (handler.guard != null && handler.guard(event, data) != true) continue;
      ...
      if (handler.isInternal) {
        ...
        if (handler.action != null) handler.action(event, data);
        return true;
      }

      _transition(handler.target,
  ...
  void _enter(List<State<S, E>> path) {
    ...
    if (this != path.last) {
      ...
      log.finer(() => '$this: entered - '
          'transition to intialState($initialState)');
  ...
  void _transition(State<S, E> nextState,
      {bool local = true, E event, data, ActionFunction<E> action}) {
    log.finer(() => '$this: _transition($nextState, '
    ...
    if (local) {
      ...
      if (this.isAncestorOf(nextState)) {
        // we are the lca.
        lca = this;
      } else
      /* this is descendant of nextState */ {
        // they are the lca.
        lca = nextState;
      }
    } else {
  ...
  static State<S, E> lowestCommonAncestor<S, E>(
      State<S, E> left, State<S, E> right) {
...
class ParallelState<S, E> extends State<S, E> {
  ...
  void _enter(List<State<S, E>> path) {
    ...
    if (activeChild == null && initialState != null) {
      log.finer(() => '$this: entered - '
          '_transition to intialState($initialState)');
      _transition(initialState, action: _handleInitialAction);
    }
  }
...
class EventHandler<S, E> {
  ...
  EventHandler(
      {this.target, this.guard, this.action, this.isLocalTransition = true});
...
typedef bool GuardFunction<E>(E event, dynamic data);
typedef void ActionFunction<E>(E event, dynamic data);
