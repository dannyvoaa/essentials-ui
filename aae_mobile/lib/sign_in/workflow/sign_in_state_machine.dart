...
class SignInStateMachine extends WorkflowStateMachineBase {
  ...
  // [isInitial] represents whether this is the initial sign in when the user
  // opens the app.
  ...
  void _setUpSilentSignIn() {
    // Entering the silent sign in state should trigger an attempt to sign in
    // silently.
    _hsm[SignInStates.silentSignIn].onEnter =
        _blocProvider.signInBloc().signInSilently;

    // Override handler for if sign in silently succeeds, in which case the
    ...
    _hsm[SignInStates.silentSignIn].addHandlers(SignInEvents.silentSignInFailed,
        [forwardTransition(SignInStates.welcomePage)]);
  }

  void _setUpWelcomePage() {
    // Handle a plugin failure by navigating to the failure page.
    _hsm[SignInStates.welcomePage].addHandlers(
  ...
  void _setUpTopicsSelection() {
    ...
    _hsm[SignInStates.topicsSelection].addHandler(
    ...
    _hsm[SignInStates.topicsSelection].addHandlers(
      SignInEvents.routePopped,
      [backwardTransition(SignInStates.loginPage)],
    );
